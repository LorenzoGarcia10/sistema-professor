...
export default function NovaProvaPage() {
  ...
  const adicionarQuestao = () => {
    if (!questaoAtual.pergunta || questaoAtual.alternativas.some((alt) => !alt.trim())) {
  ...
  const salvarProva = () => {
    if (!titulo || !descricao || questoes.length === 0) {
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) =>
                    setQuestaoAtual({ ...questaoAtual, respostaCorreta: Number.parseInt(value) })
                  }
                >
                  {questaoAtual.alternativas.map((alt, index) => (
                    <div key={index} className="flex items-center space-x-2">
                      <RadioGroupItem value={index.toString()} id={`resp-${index}`} />
                      <Label htmlFor={`resp-${index}`}>
                        {String.fromCharCode(65 + index)}) {alt || `Alternativa ${String.fromCharCode(65 + index)}`}
                      </Label>
                    </div>
                  ))}
                </RadioGroup>
                  ...
                  {questoes.map((questao, index) => (
                    <div key={questao.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <Button variant="destructive" size="sm" onClick={() => removerQuestao(questao.id)}>
                      ...
                      <div className="space-y-1">
                        {questao.alternativas.map((alt, altIndex) => (
                          <p
                            key={altIndex}
                            className={`text-sm ${altIndex === questao.respostaCorreta ? "text-green-600 font-medium" : "text-gray-600"}`}
                          >
